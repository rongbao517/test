...
def run_parallel_analysis_pipeline():
    ...
    output_file_path = "./bitcoin_news_analysis_results_full.json"
    ...
    if not prompts_to_process:
        ...
        for article, raw_result in zip(articles_to_process, batch_results):
            """
            处理模型返回：
              - 支持模型直接返回 dict；
              - 若返回字符串，尝试 json.loads；若失败，则尝试从文本中截取第一个 `{...}` 子串再解析。
              - 成功解析后，将解析出的字段合并到原始 article（保持原始所有字段），并确保保留 article_link/news_title。
            """
            parsed_result = None
            # 如果已经是 dict，直接使用
            if isinstance(raw_result, dict):
                ...
                try:
                    ...
                    # 若失败，尝试在文本中找到第一个 '{' 到最后一个 '}' 的子串再解析
                    try:
                        start = raw_result.find('{')
                        end = raw_result.rfind('}') + 1
                        if start != -1 and end != -1 and end > start:
                            candidate = raw_result[start:end]
            ...
            if parsed_result and isinstance(parsed_result, dict):
                # 合并策略：保留原始 article 的所有字段，然后把解析结果（如 relevance_to_bitcoin_price, impact_analysis 等）覆盖或加入
                merged = article.copy()
                merged.update(parsed_result)
                # 保证可追溯字段存在（article 中通常已有 title/link，但明确写入便于 downstream）
                merged['news_title'] = article.get('title', merged.get('news_title', 'No Title'))
                merged['article_link'] = article.get('link', merged.get('article_link', ''))

                all_results.append(merged)
                newly_processed_count += 1
            else:
                # 若某条分析无法解析为结构化 JSON，打印警告并保留原始文章（可选：将失败信息记录到 all_results）
                print(f"\n--- Analysis Failed or Unparsable ---")
                print(f"Article: '{article.get('title', 'No Title')}' could not be parsed from model output.")
                # 可选：将原文加上一个失败标记写入结果，便于后续重试；此处仅打印并跳过
                print("-----------------------\n")
